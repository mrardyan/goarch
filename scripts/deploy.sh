    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^([^=]+)=(.*)$ ]]; then
            ...
            case "$key" in
                ...
                JWT_SECRET|EMAIL_SMTP_USERNAME|EMAIL_SMTP_PASSWORD|DATABASE_URL|REDIS_HOST|REDIS_PORT|REDIS_PASSWORD|REDIS_DB|REDIS_TIMEOUT|REDIS_POOL_SIZE|DO_APP_ID|DO_APP_NAME|TRUSTED_PROXIES)
                    scope="RUN_TIME"
                    ;;
                # Default to runtime
                *)
